...
export async function generateVerified({
  ...
  for (let round = 1; round <= maxRounds; round++) {
    ...
    // 3. Persist outputs immediately (provider URLs expire).
    if (!state.model_glb_url) throw new Error(`Round ${round}: task succeeded but returned no model URL.`);
    const glbPath = await downloadTo(state.model_glb_url, resolve(roundDir, "model.glb"));
